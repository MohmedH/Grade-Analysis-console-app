...
public:
  ...
  {
    int N = 0;
    N = this->NumD + this->NumF + this->NumW;
    return N;
  }
...
double GetDFWRate(const std::map<string,Dept> &v, int& DFW, int& N);
...
vector<Course> DFWThresh(const std::vector<Course> &v, double tresh);
vector<Course> DFWThresh(const std::map<string,Dept> &v, double tresh);
...
GradeStats GetGradeDistribution(const std::map<string,Dept> &v);
